        ...
        <a-tabs default-active-key="basic">
          ...
          <a-tab-pane key="basic" tab="Basic Information">
            ...
            <a-row :gutter="16">
              ...
              <a-col :xs="24" :md="12">
                <a-form-item label="Select Venue" name="selected_venue_id">
                  <a-select v-model:value="selectedVenueId" placeholder="Select from existing venues (optional)" show-search
                    allow-clear :filter-option="filterOption" @change="handleVenueChange" style="width: 100%">
                    <a-select-option v-for="venue in venues" :key="venue.id" :value="venue.id">
                      {{ venue.name }} ({{ venue.city }}, {{ venue.country }})
                    </a-select-option>
                  </a-select>
                  <template #extra>
                    <span class="form-extra-text">Selecting a venue will auto-fill the address details for offline/hybrid events.</span>
                  </template>
                </a-form-item>
              </a-col>
            </a-row>
