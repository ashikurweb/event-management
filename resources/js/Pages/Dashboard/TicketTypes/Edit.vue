    ...
    <DashboardLayout>
        <div class="p-6">
            <div class="mb-6 flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit: {{ ticketType.name }}</h1>
                    <p class="text-gray-500 dark:text-gray-400">Update the ticket details and availability.</p>
                </div>
            </div>
...
const form = useForm({
    event_id: props.ticketType.event_id,
    name: props.ticketType.name,
    description: props.ticketType.description,
    type: props.ticketType.type,
    price: props.ticketType.price,
    currency: props.ticketType.currency || 'USD',
    quantity_total: props.ticketType.quantity_total,
    min_per_order: props.ticketType.min_per_order,
    max_per_order: props.ticketType.max_per_order,
    sale_start: props.ticketType.sale_start,
    sale_end: props.ticketType.sale_end,
    is_visible: props.ticketType.is_visible,
    requires_approval: props.ticketType.requires_approval,
    absorb_fees: props.ticketType.absorb_fees,
    display_order: props.ticketType.display_order
});

const submit = () => {
    form.put(`/dashboard/ticket-types/${props.ticketType.id}`, {
        onSuccess: () => {
