...
const existingLogo = computed(() => {
  ...
  return vendor.value.logo_url || null;
});
...
onMounted(() => {
  if (vendor.value) {
    form.name = vendor.value.name || '';
    form.company = vendor.value.company || '';
    form.email = vendor.value.email || '';
    form.phone = vendor.value.phone || '';
    form.description = vendor.value.description || '';
    form.website = vendor.value.website || '';
    form.category = vendor.value.category || '';
    form.rating = vendor.value.rating || 0.00;
    form.is_verified = vendor.value.is_verified || false;
  }
});
...
const handleSubmit = async () => {
  try {
    ...
    router.post(`/dashboard/vendors/${vendor.value.id}`, formData, {
      preserveScroll: true,
      forceFormData: true,
      _method: 'PUT',
      onSuccess: () => {
