<template>
  <DashboardLayout>
    ...
    <a-card class="team-invitation-form-card">
      <template #title>
        <h2 class="card-title">Edit Team Invitation</h2>
      </template>
...
const teamInvitation = computed(() => page.props.teamInvitation || {});
...
// Initialize form with team invitation data
watch(() => teamInvitation.value, (newValue) => {
  if (newValue && Object.keys(newValue).length > 0) {
    ...
    form.expires_at = newValue.expires_at ? dayjs(newValue.expires_at) : null;
    form.status = newValue.status || 'pending';
  }
}, { immediate: true });
...
const handleSubmit = async () => {
  try {
    ...
    router.put(`/dashboard/team-invitations/${teamInvitation.value.id}`, submitData, {
      preserveScroll: true,
      onSuccess: () => {
