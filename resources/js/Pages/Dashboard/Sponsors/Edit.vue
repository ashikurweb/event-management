<template>
  <DashboardLayout>
    ...
    <a-card class="sponsor-form-card">
      <template #title>
        <h2 class="card-title">Edit Sponsor</h2>
      </template>
      ...
      >
        ...
        <a-form-item label="Logo" name="logo">
          <div v-if="existingLogo" class="existing-logo">
            <a-image
              :src="existingLogo"
              :width="150"
              :preview="false"
...
const existingLogo = computed(() => {
  ...
  return sponsor.value.logo_url || null;
});
...
onMounted(() => {
  if (sponsor.value) {
    form.name = sponsor.value.name || '';
    form.tier = sponsor.value.tier || 'partner';
    form.description = sponsor.value.description || '';
    form.website = sponsor.value.website || '';
    form.display_order = sponsor.value.display_order || 0;
    form.is_active = sponsor.value.is_active !== undefined ? sponsor.value.is_active : true;
  }
});
